...
@REM set TMPSG=X:\install\msvc%VCVERS%0-64\simgear
@set TMP3RD=X:\3rdParty.x64
@REM set TMPBOOST=c:\local\boost_1_62_0
@REM set TMPOSG=%TMPDRV%\install\msvc100-64\OpenSceneGraph
@REM set TMPBOOST=X:\install\msvc%VCVERS%0-64\boost
@if NOT EXIST %TMP3RD%\nul goto NO3RD
@REM if NOT EXIST %TMPSG%\nul goto NOSG
@REM if NOT EXIST %TMPBOOST%\nul goto NOBOOST
...
@REM set TMPOPTS=%TMPOPTS% -DBUILD_UTILS:BOOL=OFF
...
@REM echo.
@REM echo Check %TMPLOG% to ensure 3rdparty items found - *** Only Ctrl+C aborts ***
@REM echo.
@REM %DOPAUSE%
